...
$.fn.TableCheckAll = function( options ) {
  ...
  return this.each( function() {
    ...
    $( this ).find( settings.checkboxClass ).on( "click", function() {
      const totalCheckbox = $( this ).parents( "table" ).find( settings.checkboxClass ).length;
      const totalChecked = $( this ).parents( "table" ).find( `${ settings.checkboxClass }:checked` ).length;
